...
private:
    ...
    void InitializeKeyboard() {
        ESP_LOGI(TAG, "Initialize 74HC138 keyboard");
        keyboard_ = new Hc138Keyboard(KEYBOARD_HC138_A0, KEYBOARD_HC138_A1, KEYBOARD_HC138_A2);
    ...
    void HandleKeyEvent(const KeyEvent& event) {
        ...
        // Handle W and S keys during WiFi configuring state
        auto& app = Application::GetInstance();
...
public:
    M5StackCardputerV11Board() : boot_button_(BOOT_BUTTON_GPIO) {
        InitializeSpi();
    ...
    virtual AudioCodec* GetAudioCodec() override {
        // Cardputer v1.1 uses PDM mic + I2S speaker (no codec chip)
        // GPIO43 is shared: PDM mic clock (I2S_NUM_0) and I2S speaker WS (I2S_NUM_1)
        static NoAudioCodecSimplexPdm audio_codec(
            AUDIO_INPUT_SAMPLE_RATE,
            AUDIO_OUTPUT_SAMPLE_RATE,
            AUDIO_I2S_SPK_GPIO_BCLK,   // spk_bclk = GPIO41
            AUDIO_I2S_SPK_GPIO_WS,     // spk_ws = GPIO43
            AUDIO_I2S_SPK_GPIO_DOUT,   // spk_dout = GPIO42
            AUDIO_I2S_MIC_GPIO_SCK,    // mic_sck = GPIO43 (PDM clock)
            AUDIO_I2S_MIC_GPIO_DIN);   // mic_din = GPIO46
        return &audio_codec;
    }
...
DECLARE_BOARD(M5StackCardputerV11Board);
